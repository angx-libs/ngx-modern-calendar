export class ModernCalendar {
    ...
    constructor(options?: any) {
        ...
        this.opts.shortWeekday = this.localeData.days.sort((d1: { priority: number; }, d2: { priority: number; }) => d1.priority - d2.priority).map((d: { symbol: any; }) => d.symbol);
    ...
    private getWeekDay = (day: string) => {
        return this.localeData.days.sort((d1: { priority: number; }, d2: { priority: number; }) => d1.priority - d2.priority).map((d: { longName: any; }) => d.longName)[day];
    }
    ...
    private createDay = (date: Date) => {
        ...
        const availableWeekDay = this.opts.availableWeekDays.filter((f: { day: number; }) => f.day === date.getDay() ||
            f.day === this.getWeekDay(date.getDay().toString()));
        const availableDate = this.opts.availableDates.filter((f: { date: string; }) => f.date === (date.getFullYear() +
    ...
    private selectDate = () => {
        ...
        activeDates.forEach((date: { addEventListener: (arg0: string, arg1: (event: any) => void) => void; }) => {
            date.addEventListener('click', (event: { dataset: any; target: { classList: { add: (arg0: string) => void; }; }; }) => {
    ...
    private setWeekDayHeader = () => {
        (document.querySelector(`${this.opts.selector} .modern-calendar-week`) ?? new Element()).innerHTML = `
